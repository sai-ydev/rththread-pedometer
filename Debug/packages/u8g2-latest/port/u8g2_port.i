# 1 "../packages/u8g2-latest/port/u8g2_port.c"
...
# 1 "<command-line>" 2
# 1 "../packages/u8g2-latest/port/u8g2_port.c"
# 1 "../packages/u8g2-latest/port/u8g2_port.h" 1
...
# 9 "../packages/u8g2-latest/port/u8g2_port.h" 2
...
# 10 "../packages/u8g2-latest/port/u8g2_port.h" 2
...
# 15 "../packages/u8g2-latest/port/u8g2_port.h" 2
...
# 16 "../packages/u8g2-latest/port/u8g2_port.h" 2
# 1 "C:\\Users\\yaman\\OneDrive\\Documents\\rtthread-workspace\\CH32V208\\packages\\u8g2-latest\\src/u8x8.h" 1
# 17 "../packages/u8g2-latest/port/u8g2_port.h" 2

#define ARRAY_SIZE(a) (sizeof(a) / sizeof((a)[0]))
# 40 "../packages/u8g2-latest/port/u8g2_port.h"
...
# 2 "../packages/u8g2-latest/port/u8g2_port.c" 2


#define MAX_RETRY 3


static struct rt_i2c_bus_device *i2c_bus = 0;
# 55 "../packages/u8g2-latest/port/u8g2_port.c"
uint8_t u8x8_gpio_and_delay_rtthread(u8x8_t *u8x8, uint8_t msg, uint8_t arg_int, void *arg_ptr)
{
    uint8_t i;
    switch(msg)
    {
        ...
        case 40:
            ...
            rt_pin_mode(u8x8->pins[0],0x00);
            rt_pin_mode(u8x8->pins[1],0x00);
            rt_pin_mode(u8x8->pins[11],0x00);
            rt_pin_mode(u8x8->pins[10],0x00);
            rt_pin_mode(u8x8->pins[9],0x00);


            rt_pin_mode(u8x8->pins[13],0x00);
            rt_pin_mode(u8x8->pins[12],0x00);


            rt_pin_mode(u8x8->pins[0],0x00);
            rt_pin_mode(u8x8->pins[1],0x00);
            rt_pin_mode(u8x8->pins[2],0x00);
            rt_pin_mode(u8x8->pins[3],0x00);
            rt_pin_mode(u8x8->pins[4],0x00);
            rt_pin_mode(u8x8->pins[5],0x00);
            rt_pin_mode(u8x8->pins[6],0x00);
            rt_pin_mode(u8x8->pins[7],0x00);
            rt_pin_mode(u8x8->pins[8],0x00);
            rt_pin_mode(u8x8->pins[10],0x00);
            rt_pin_mode(u8x8->pins[11],0x00);


            rt_pin_mode(u8x8->pins[19],0x02);
            rt_pin_mode(u8x8->pins[16],0x02);
            rt_pin_mode(u8x8->pins[18],0x02);
            rt_pin_mode(u8x8->pins[17],0x02);
            rt_pin_mode(u8x8->pins[20],0x02);
            rt_pin_mode(u8x8->pins[21],0x02);


            rt_pin_write(u8x8->pins[0],1);
            rt_pin_write(u8x8->pins[1],1);
            rt_pin_write(u8x8->pins[11],1);
            rt_pin_write(u8x8->pins[10],1);
            rt_pin_write(u8x8->pins[9],1);
            break;
        ...
        case (64+(2)):
            rt_pin_write(u8x8->pins[2],arg_int);
            break;

        case (64+(3)):
            rt_pin_write(u8x8->pins[3], arg_int);
            break;

        case (64+(4)):
            rt_pin_write(u8x8->pins[4], arg_int);
            break;

        case (64+(5)):
            rt_pin_write(u8x8->pins[5], arg_int);
            break;

        case (64+(6)):
            rt_pin_write(u8x8->pins[6], arg_int);
            break;

        case (64+(7)):
            rt_pin_write(u8x8->pins[7], arg_int);
            break;

        case (64+(8)):
            rt_pin_write(u8x8->pins[8], arg_int);
            break;

        case (64+(12)):


            rt_pin_write(u8x8->pins[12], arg_int);
            break;

        case (64+(13)):


            rt_pin_write(u8x8->pins[13], arg_int);
            break;

        case (64+(0)):

            rt_pin_write(u8x8->pins[0], arg_int);
            break;

        case (64+(1)):

            rt_pin_write(u8x8->pins[1], arg_int);
            break;

        case (64+(9)):

            rt_pin_write(u8x8->pins[9], arg_int);
            break;

        case (64+(10)):

            rt_pin_write(u8x8->pins[10], arg_int);
            break;

        case (64+(11)):

            rt_pin_write(u8x8->pins[11], arg_int);
            break;

        default:
            if ( msg >= (64+(0)) )
            {
                i = ((u8x8)->pins[(msg)&0x3f]);
                if ( i != 255 )
                {
                    if ( ((msg)&0x3f) < 16 )
                    {
                    ...
                    else
                    {
                        ...
                        ((u8x8)->gpio_result = (rt_pin_read(i) == 0 ? 0 : 1));
                    }
                }
                break;
            }
...
uint8_t u8x8_byte_rtthread_hw_i2c(u8x8_t *u8x8, uint8_t msg, uint8_t arg_int, void *arg_ptr)
{
    ...
    switch(msg)
    {
        ...
        case 25:
            ...
            msgs.addr = ((u8x8)->i2c_address)>>1;
            msgs.flags = 0x0000;
            msgs.buf = buffer;
            msgs.len = buf_idx;
            while(rt_i2c_transfer(i2c_bus, &msgs, 1) != 1 && t < 3)
            {
                t++;
            };
            if(t >= 3)
